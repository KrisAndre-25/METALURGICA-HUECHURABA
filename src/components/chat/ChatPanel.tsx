...
export function ChatPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const subtitle = user?.role === 'CLIENT' ? t.chat.subtitleClient : t.chat.subtitleInternal;

  return (
    <BottomSheet open={open} onClose={onClose} title={t.chat.title} subtitle={subtitle}>
      <div className="flex h-[60vh] flex-col">
        <div ref={listRef} className="flex-1 space-y-3 overflow-y-auto pr-1">
          {messages.length === 0 && (
            <p className="py-10 text-center text-sm text-forge-steel">
              {user?.role === 'CLIENT' ? t.chat.emptyClient : t.chat.emptyInternal}
            </p>
          )}
          {messages.map((m) => {
              ...
              <div key={m.id} className={cn('flex flex-col gap-1', isOwn && 'items-end')}>
                <div className={cn(
                  ...
                  <div className="mb-1 flex items-center gap-1.5">
                    ...
                    <span className={cn('text-[10px] font-medium', ROLE_TEXT[m.senderRole])}>{formatRole(m.senderRole, language)}</span>
                  </div>
                ...
                <span className="px-1 text-[10px] text-forge-steel">{formatRelativeTime(m.timestamp, language)}</span>
              </div>
            );
          })}
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-3 shrink-0 space-y-2 border-t border-forge-border pt-3">
          {showOrderPicker && (
            <select
              aria-label={t.chat.referenceOrder}
              value={orderId}
            ...
            >
              <option value="">{t.chat.noOrderReferenced}</option>
              {allOrders.map((o) => (
